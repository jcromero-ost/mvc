document.addEventListener("DOMContentLoaded", function () {  
    ...
    detenerBtn.addEventListener('click', () => {
        ...
        if (!horaInicio) {
            console.error("No se ha definido la hora de inicio.");
            return;
        }

        // Calcular el tiempo entre horaInicio y horaFin con la función existente
        const tiempo = calcularTiempo(horaInicio, horaFin);
        ...
        formData.append('tiempo', tiempo); // Tiempo calculado entre horaInicio y horaFin
        formData.append('hora_inicio', horaInicio); // Hora inicio
        formData.append('hora_fin', horaFin); // Hora fin
    ...
    window.addEventListener('beforeunload', function (event) {
        if (isRunning) {
            ...
            // Guardar la hora de fin justo antes de salir
            const horaFin = obtenerHoraFormato();

            const tiempo = timerDisplay.querySelector('p').textContent;
            const fecha = new Date().toISOString().split('T')[0];

            // Si no hay horaInicio (por algún motivo), la igualamos a horaFin para evitar null
            if (!horaInicio) {
                horaInicio = horaFin;
            }
            ...
            // Para enviar datos justo antes de salir se usa navigator.sendBeacon si está disponible,
            // porque fetch puede no completarse si la pestaña se cierra rápido.

            const url = '/store_ticket_cronometro';

            if (navigator.sendBeacon) {
                // Crear un Blob con los datos formData
                const plainFormData = new URLSearchParams();
                for (const pair of formData.entries()) {
                    plainFormData.append(pair[0], pair[1]);
                }
                const blob = new Blob([plainFormData.toString()], { type: 'application/x-www-form-urlencoded' });

                navigator.sendBeacon(url, blob);
            } else {
                // Fallback: request síncrono (menos recomendado)
                const xhr = new XMLHttpRequest();
                xhr.open('POST', url, false); // false para síncrono
